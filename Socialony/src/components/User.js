...
const User = (props) => {
  ...
  const isFollowBack = loggedUser?.followers?.includes(profileId)
    ? true
    : false;
  ...
  const RenderPosts = () => {
    if (user) {
      return user.posts.map((post, index) => (
        <Post key={index} user={user?.credentials} post={post} />
      ));
    }
  };
  ...
  const followUser = () => {
    ...
    const url = `https://us-central1-socialony.cloudfunctions.net/api/user/${profileId}/${
    ...
    axios({
      ...
      .then(() => {
        message.success({
          content: `User ${!isUserFollow ? "Followed!" : "Unfollowed!"}`,
          key,
          duration: 2,
        });
      })
  ...
  const imageUpload = (e) => {
    ...
    axios
      .post(
        `https://us-central1-socialony.cloudfunctions.net/api/user/${profileId}/image`,
        formData
      )
      .then((respond) => {
      ...
      .catch((e) => {
        console.log(e);
        message.error({
          content: "Image Could Not Uploaded!",
          key,
          duration: 2,
        });
      });
          ...
          <Grid item xs={6} className={classes.buttongrid}>
            ...
            >
              ...
              <Modal.Actions>
                ...
                <Btn onClick={sendNewMessage} positive>
                  Send
                </Btn>
              </Modal.Actions>
            </Modal>
      ...
      return user.credentials.follows.map((follow, index) => {
        return (
          <Grid
            ...
            lg={6}
            key={index}
            onClick={() => setOpenFollowsModal(false)}
          >
          ...
          >
              ...
              >
                ...
                <Modal.Content image scrolling>
                  <List style={{ width: "100%" }}>
                    <Grid container>{renderFollows()}</Grid>
                  </List>
                </Modal.Content>
              </Modal>
            ...
            <Grid item xs={4} lg={2}>
              <Modal
                ...
                trigger={
                  <p className={classes.stats}>
                    {user?.credentials.followersCount}
                  </p>
                }
              >
                <Modal.Header className={classes.modalheader}>
                  {user?.credentials.name} {user?.credentials.surname}'s
                  Followers
                  <Icon
                    className="close"
                    onClick={() => setOpenFollowersModal(false)}
                ...
                <Modal.Content image scrolling>
                  <List style={{ width: "100%" }}>
                    <Grid container>{renderFollowers()}</Grid>
                  </List>
                </Modal.Content>
              </Modal>
